...
/* Function Bodies */
static float chess_storage(%chess_alignof(v8float))  vsi_aibe_vec_const_0[8] = {6.283185e+00, 6.283185e+00, 6.283185e+00, 6.283185e+00, 6.283185e+00, 6.283185e+00, 6.283185e+00, 6.283185e+00};

void AIE_mulTest(input_stream_float *  restrict  A, output_stream_float *  restrict  C) {
  uint64_t index;
  uint64_t index__PHI_TEMPORARY;
v8float wide_load;
for (int i=0; i< 8; i++) {
       wide_load = upd_elem(wide_load, i, readincr(A));
}
//  for (int i=0; i< 8; i++) {
//           printf("A[%d] = %f \n", i, ext_elem(wide_load, i));
//    }
#line 1 "/net/shared/kafi/AIE_mulTest/AIE_mulTest.cc"
  ;
  ;
#line 2 "/net/shared/kafi/AIE_mulTest/AIE_mulTest.cc"
  ;
#line 4 "/net/shared/kafi/AIE_mulTest/AIE_mulTest.cc"
  ;
...
#line 5 "/net/shared/kafi/AIE_mulTest/AIE_mulTest.cc"
//    wide_load = *(v8float*)((float*)get_wp_float(A,index));
  
// v8float  store_temp_0; // = (v8float*)((float*)get_wp_float(C,index));
 v8float  store_temp_0 = as_v8float(aie::mul(aie::vector<float,8>(wide_load),aie::vector<float,8>((*(v8float *)vsi_aibe_vec_const_0))));
//    for (int i=0; i< 8; i++) {
//           printf("C[%d] = %f \n", i, ext_elem(store_temp_0, i));
//    }
   for (int i=0; i< 8; i++) {
          writeincr(C, ext_elem(store_temp_0, i));
