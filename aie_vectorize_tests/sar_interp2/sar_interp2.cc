...
typedef struct {
	_complex resampled[NUMBER_PULSES_FOR_EACH_AIE*PFA_NOUT_RANGE];
} Interp2DataOut;
...
void sar_interp2(
    ...
    _complex accum_arr_first[NUMBER_PULSES_FOR_EACH_AIE];
    _complex* __restrict__ accum_arr = &accum_arr_first[0];
    ...
    for (r = 0; r < 8; ++r)
    {
        ...
        for (p = 0; p < 8; ++p)
        {
            ...
            // for (k = 0; k < NUMBER_PULSES_FOR_EACH_AIE; ++k)
            // {
            //     accum.re += accum_arr[k].re;
            //     accum.im += accum_arr[k].im;
            //     // accum +=accum_arr[k];
            // }
