...
#include <chrono>
std::chrono::duration<double,std::milli> xfer_time = std::chrono::duration<double,std::milli>::zero();
...
void send_recv_wave_fft(hls::stream<fft_data_s> &fft_ds_o,hls::stream<fft_control_s> &control,hls::stream<fft_data_s> &fft_ds_i)
{
	wav_hdr wavHeader;
	wav_gen_hdr wavChdr;
	FILE *wavFile;
	...
	int s_size = wavHeader.bitsPerSample;
	while (!feof(wavFile)) {
		ps_size++;
		if (s_size == 8) {
			unsigned char d;
			fread(&d,sizeof(d),1,wavFile);
			fd.data.re = (float)d;
			fd.data.im = 0.0 ;
		} else if (s_size == 16) {
			unsigned short d;
		...
		if (fd.last) t1 = std::chrono::high_resolution_clock::now();
		fft_ds_o.write(fd); // send it

		// send the packet now wait for the response
		if (fd.last) {
			struct fft_data_s fd_i;
			ps_size = 0;
			do {
				fd_i = fft_ds_i.read();
			} while (!fd_i.last); // read till end of packet
			t2 = std::chrono::high_resolution_clock::now();
			xfer_time = t2 - t1;
			std::cout << "time taken " << xfer_time.count() << " ms" << std::endl;
		}
	}	
	if (ps_size == p_size) exit(0); // done
...
static void send_wave_fft (hls::stream<fft_data_s> &fft_ds, hls::stream<int> &cont, FILE *wavf, int s_size, int p_size)
{
	int pb_size = 2;
	int ps_size = 0;
	struct fft_data_s fd;
